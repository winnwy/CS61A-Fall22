...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if (not ant_here) or (ant_here and not ant_here.blocks_path):
            return False
        return True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while p and not p.is_hive:
            if p.bees:
                for b in p.bees:
                    dict_insects[b] = distance
            if p.ant:
                dict_insects[p.ant] = distance 
                if p.ant.is_container and p.ant_contained:
                    dict_insects[p.ant_contained] = distance
            p = p.entrance
            distance += 1
        return dict_insects
        # END Problem Optional 2
