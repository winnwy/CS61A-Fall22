...
def do_define_macro(expressions, env):
    ...
    signature = expressions.first
    if scheme_symbolp(signature):
        # assigning a name to a value e.g. (define-macro x (+ 1 2))
        validate_form(expressions, 2, 2)  # Checks that expressions is a list of length exactly 2
        env.bindings[signature] = scheme_eval(expressions.rest.first, env)
        return signature
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # macro defining a named procedure e.g. (define-macro (f x y) (+ x y))
        symbol = expressions.first.first
        formals = expressions.first.rest
        body = expressions.rest
        env.bindings[symbol] = MacroProcedure(formals, body, env)
        return symbol
    else:
        bad_signature = signature.first if isinstance(signature, Pair) else signature
        raise SchemeError('non-symbol: {0}'.format(bad_signature))
