...
def accuracy(typed, source):
    ...
    if typed_length == source_length == 0:
        return 100.0
    elif typed_length == 0 or source == 0:
        return 0.0
    elif typed_length > source_length:
        compare_length = source_length
    else:
        compare_length = typed_length
    count = len([i for i in range(compare_length) if typed_words[i] == source_words[i]])
...
def wpm(typed, elapsed):
    ...
    group_num = 5
    one_minute = 60
    return len(typed) / group_num * (one_minute / elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    diff_words = [diff_function(typed_word, w, limit) for w in word_list]
    min_diff = min(diff_words)
    if typed_word in word_list or min_diff > limit:
        return typed_word
    else:
        min_diff_index = diff_words.index(min_diff)
...
def final_diff(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:  # Fill in the condition
        ...
        # BEGIN
        return 1 + min(final_diff(add[1:], source[1:], limit - 1),
                    final_diff(remove, source, limit - 1),
                    final_diff(substitute[1:], source[1:], limit - 1))
        # END
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for typed_word in typed:
        if typed_word == prompt[i] and i < len(typed):
            count += 1
            i += 1
        else:
            break
    correct_ratio = count / len(prompt)
...
def fastest_words(match):
    ...
    for p in player_indices:
        players_min_time.append([])
    for w in word_indices:
        min_time = all_times[0][w] + 1
        index = 0
        for p in player_indices:
            if all_times[p][w] < min_time:
                min_time = all_times[p][w]
                index = p
        players_min_time[index].append(get_word(match, w))
    return players_min_time
