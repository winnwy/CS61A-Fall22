...
def pingpong(n):
    ...
    True
    """
    def contains_eight(i):
        if i == 0:
            return False
        if i % 10 == 8:
            return True
        else:
            return contains_eight(i // 10)

    def change_direction(k):   
        if contains_eight(k) or k % 8 == 0:
            return True
        else:
            return False
    ...
    def direction(k):
        if k == 1:
            return 1
        elif change_direction(k):
            return (-1)*direction(k - 1)
        else:
...
def count_coins(change):
    ...
    def use_coins(change, coin):
        if change == 0:
            ...
            return use_coins(change - coin, coin) + use_coins(change, next_smaller_coin(coin))
            
    return use_coins(change, 25)
